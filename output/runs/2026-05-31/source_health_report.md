# Source Health Report

Generated: 2026-05-31 23:01:56

- Total sources attempted: 163
- Working sources: 24
- Failing sources: 13
- Placeholder sources: 0
- Disabled registered sources: 126

## High-Value Sources

- Multifamily Executive (Core Multifamily News): 3 saved article(s).
- Yield PRO (Core Multifamily News): 10 saved article(s).
- Commercial Observer (Core Multifamily News): 4 saved article(s).
- Connect CRE (Core Multifamily News): 3 saved article(s).
- NAHB Eye on Housing - Multifamily (Public Agency / Housing Data): 15 saved article(s).
- Urbanize LA (Regional / California / LA Sources): 8 saved article(s).
- REBusiness Online (Site / Parcel Source Expansion): 4 saved article(s).
- Connect CRE Apartments (Site / Parcel Source Expansion): 6 saved article(s).
- Connect CRE South Florida (Site / Parcel Source Expansion): 6 saved article(s).
- Connect CRE Atlanta (Site / Parcel Source Expansion): 3 saved article(s).
- Connect CRE Charlotte (Site / Parcel Source Expansion): 3 saved article(s).
- Connect CRE Orange County (Site / Parcel Source Expansion): 3 saved article(s).
- Connect CRE California (Regional / California / LA Sources): 5 saved article(s).

## Useful Sources

- Multifamily Dive (Core Multifamily News): 2 saved article(s).
- Bisnow (Core Multifamily News): 2 saved article(s).
- HousingWire (Core Multifamily News): 1 saved article(s).
- Blackstone Real Estate (Developer / GP Newsrooms): 1 saved article(s).
- Connect CRE Phoenix (Site / Parcel Source Expansion): 2 saved article(s).
- Blackstone Real Estate Source Expansion (Developer / GP Source Expansion): 1 saved article(s).

## Failing Sources

- Multi-Housing News: RSS parsing issue
- GlobeSt: RSS parsing issue
- The Real Deal: RSS parsing issue
- Urban Land Institute: RSS parsing issue
- NMHC News: RSS parsing issue
- Walker & Dunlop Insights: RSS parsing issue
- Berkadia Research: RSS parsing issue
- Hines Newsroom: RSS parsing issue
- California YIMBY: RSS parsing issue
- Hines Source Expansion: RSS parsing issue
- Berkadia Source Expansion: RSS parsing issue
- Walker & Dunlop Source Expansion: RSS parsing issue
- Commercial Property Executive: RSS parsing issue

## Sources Needing Manual Review

- No placeholder sources need manual RSS review.

## Watch List

- Construction Dive: 10 entries found, but no articles passed filtering.
- Federal Reserve - Press Releases: 20 entries found, but no articles passed filtering.
- SF YIMBY: 5 entries found, but no articles passed filtering.
- Connect CRE Texas: 10 entries found, but no articles passed filtering.
- WealthManagement Real Estate: 50 entries found, but no articles passed filtering.

## GP Source Expansion Summary

Developer / GP source coverage is available in `gp_source_coverage_report.md`.
- GP source expansion rows tracked: 78
- Working GP source expansion feeds: 1
- GP source expansion placeholders/manual review: 77

## Source Activation Summary

Validated source activation scoring is available in `source_activation_report.md`.
- Working activated sources: 24
- Failed critical sources: 11
- Highest quality source this run: Connect CRE (99).

## Recommended Source Improvement Actions

- Confirm placeholder feed URLs for developer, GP, brokerage, and public-agency sources.
- Replace failing URLs with confirmed public RSS or newsroom feeds when available.
- Keep source-category bonuses modest so strong sources improve prioritization without overriding article relevance.
- Review High Value and Useful sources first when improving GP, capital-flow, and relationship intelligence.
